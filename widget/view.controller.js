...
(function () {
  angular
    ...
    playbookButtons100Ctrl.$inject = ['$scope', '_', 'currentPermissionsService', 'FormEntityService', 'playbookService', '$filter', 'jsonToGridService'];
  
    function playbookButtons100Ctrl($scope, _, currentPermissionsService, FormEntityService, playbookService, $filter, jsonToGridService) {
          ...
          $scope.actionButtonPlaybooks.push({
            ...
            onClick: function () {
              if($scope.config.showExecutionProgress){
                jsonToGridService.launchStandaloneWidget($scope.config.widgetName, $scope.config.widgetVersion, playbook, true, $scope.getExecuteRecord).then(function () {
                angular.noop;
              });
              }
              else{
